...
//! （add/list/remove 排期，绑定会话 origin）。[`mcp`] 提供 MCP 工具接入的纯变换核心
//! （工具名净化/限长、OpenAI schema 归一、畸形进度通知检测）；真实 MCP 连接/会话/传输、
//! apply_patch/find_files/web、async 执行、上下文变量注入的完整链路留待后续。
