...
#[tokio::test]
async fn find_match_exact_no_match() {
...
#[tokio::test]
async fn find_match_line_trim_fallback_returns_original_indent() {
...
#[tokio::test]
async fn find_match_line_trim_multiple_candidates() {
    let (_m, count) = find_match("  a\n  b\n  a\n  b\n", "a\nb");
    assert_eq!(count, 2);
}

#[tokio::test]
async fn find_match_empty_old_text() {
...
#[tokio::test]
async fn edit_exact_match_replaces_and_reports_success() {
...
#[tokio::test]
async fn edit_line_trim_fallback_replaces_indented_block() {
    let dir = tempdir().unwrap();
    std::fs::write(
...
#[tokio::test]
async fn edit_ambiguous_match_warns_without_replace_all() {
    let dir = tempdir().unwrap();
...
#[tokio::test]
async fn edit_missing_new_text_returns_clear_error() {
    let dir = tempdir().unwrap();
