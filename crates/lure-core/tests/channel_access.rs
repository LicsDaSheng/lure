...
#[tokio::test]
async fn star_allows_all() {
    let p = policy(&["*"]);
    assert!(p.is_allowed_no_pairing("anyone"));
}

#[tokio::test]
async fn empty_allow_from_denies() {
    let p = policy(&[]);
    assert!(!p.is_allowed_no_pairing("alice"));
}

#[tokio::test]
async fn pairing_fallback_allows_approved_only() {
...
#[tokio::test]
async fn star_wins_over_pairing_denial() {
    let p = policy(&["*"]);
    struct DenyAll;
...
#[tokio::test]
async fn config_supports_snake_and_camel_case_alias() {
...
#[tokio::test]
async fn config_null_allow_from_denies() {
...
#[tokio::test]
async fn config_missing_allow_from_defaults_empty() {
