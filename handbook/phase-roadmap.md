...
| Phase 3 Agent Loop | partial | AgentLoop/AgentRunner/ContextBuilder、EchoProvider、CLI one-shot+interactive+streaming | slash commands |
...
| Phase 6 Memory | partial | MemoryStore（读写/历史/迁移）、dream consolidation（FakeRunner）、AgentLoop/CLI 记忆接入 | 真实 LLM dream、触发策略、SOUL/USER 整合 |
...
| Phase 10 WebUI | partial | **desktop 闭环**——vendor React 前端、wry 窗口、loopback HTTP/WS、agent loop 每连接独立 | transcript/webui-thread、/api 大表面、非 macOS |
