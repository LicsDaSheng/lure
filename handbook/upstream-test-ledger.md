...
| `nanobot/channels/websocket/runtime.py`（事件协议 + channel 化） | 10 | `crates/lure-core/tests/webui_protocol.rs` + `crates/lure-core/tests/channel.rs` | partial | message/delta/status/error 事件形状与入站校验已覆盖；**服务端主动推送**（`webui::hub::WsHub` + 连接读循环 drain：cron 产出实时推给在线连接，`webui_ws_server.rs::hub_push_reaches_attached_connection` + headless 端到端）已覆盖；**Stage 5 channel 化**（`TurnEventRegistry` 按 turn_id 路由 turn 事件、`BusTurnRunner` 发布到共享 bus + 等待事件流替代每连接 AgentLoop、调度核心 `process_streaming` 流式转发、mux 事件序列经 bus runner 保持不变、chat/cron 共用单实例调度核心）`crates/lure-core/tests/channel.rs` 已覆盖；SSL、媒体待补 |
...
| 上游测试 | 归属 phase | Rust 测试 | 状态 | 说明 |
|---|---:|---|---|---|
|| `tests/test_openai_api.py` | 9 | `crates/lure-core/tests/api_openai.rs` | partial | error json、chat completion 形状/usage（真实回填 loop 累加 usage：prompt/completion/total）、单条 user 校验、model 不匹配、鉴权、固定 session、`/v1/models`（形状+鉴权）、per-session 锁原语已覆盖；media、锁接入 handler 待补。**Stage 7：/v1 HTTP server（`api_server.rs`）未接线生产入口已删除**，传输无关表面由 api_openai.rs 守护；如需 OpenAI-compat 出站端点可基于 axum 重建 |
| `tests/test_api_stream.py` | 9 | `crates/lure-core/tests/api_openai.rs` | partial | SSE 事件顺序（内容→finish→[DONE]）、逐 token 流式（逐 delta chunk、跨 tool 轮不关流、单 id、默认回退单 chunk）已覆盖；token 级 usage 待补。**Stage 7：/v1 HTTP server 删除（见上）** |
...
| `nanobot/cron/service.py`（timer/_on_timer/_execute_job） | 8 | `crates/lure-core/tests/cron_service.rs` + `crates/lure-core/tests/cron_async.rs` + `crates/lure-desktop/tests/headless.rs` | partial | `CronService::tick`（到期执行 + record_run 推进/删一次性/error 状态/禁用跳过）、`next_wake_ms`、**Stage 4 `AsyncCronScheduler` tokio interval task（非后台线程）**、**cron submit 语义（`cron_submit_message` → `AsyncBus` → 共享 `AgentLoopScheduler`）**、**调度器 `on_completed` completion 钩子（回复文本回投）**、**trigger 异步投递循环（claim → deliver → complete / recover）**、**lure-desktop 常驻 cron → transcript 写入 headless 验证**、**cron → 在线 WS 连接实时推送（WsHub，headless 端到端）**已覆盖；run history、精确睡到 next_wake 待补。**Stage 7：同步线程版 `CronScheduler` 与退役的 `SessionBusy` 已删除**（async interval task 到期语义由 cron_async.rs 覆盖） |
| `tests/triggers/test_local_triggers.py` | 8 | `crates/lure-core/tests/trigger_queue.rs` + `crates/lure-core/tests/cron_async.rs` | partial | enqueue/claim/complete/recover(at-least-once)/忙等/limit 已覆盖；**Stage 4 异步投递循环（`run_delivery_once`：跳过忙 session、deliver 失败 reenqueue 回 pending）**已覆盖；文件 inbox 布局、trigger 定义存储待补 |
