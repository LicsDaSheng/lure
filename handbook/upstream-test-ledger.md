...
| `nanobot/cron/service.py`（timer/_on_timer/_execute_job） | 8 | `crates/lure-core/tests/cron_service.rs` + `crates/lure-core/tests/cron_async.rs` + `crates/lure-desktop/tests/headless.rs` | partial | `CronService::tick`（到期执行 + record_run 推进/删一次性/error 状态/禁用跳过）、`next_wake_ms`、`CronScheduler` 工厂式后台轮询、**Stage 4 `AsyncCronScheduler` tokio interval task（非后台线程）**、**cron submit 语义（`cron_submit_message` → `AsyncBus` → 共享 `AgentLoopScheduler`）**、**`SessionBusy` defer 协调（chat turn 活跃时让位）**、**调度器 `on_completed` completion 钩子（回复文本回投）**、**trigger 异步投递循环（claim → deliver → complete / recover）**、**lure-desktop 常驻 cron → transcript 写入 headless 验证**、**cron → 在线 WS 连接实时推送（WsHub，headless 端到端）**已覆盖；run history、精确睡到 next_wake 待补 |
